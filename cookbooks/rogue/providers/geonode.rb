...
def set_perms(directory, perm=570)
  bash "set_file_permissions" do
    code "chmod #{perm} #{directory} -R && chown www-data:#{$rogue_os_usr} #{directory} -R"
  end
end
...
action :install do
  if test
    ...
    bash "virtual_env permissions" do
      code "chmod 570 #{new_resource.virtual_env_location} -R && chown www-data:#{$rogue_os_usr} #{new_resource.virtual_env_location} -R"
    end
...
action :build_html_docs do
  bash "build docs" do
    code "source #{new_resource.virtual_env_location}/bin/activate && cd #{new_resource.rogue_geonode_location}/docs && make html && chmod 574 build -R && chown www-data:#{$rogue_os_usr} build -R"
