...
if node['tomcat']['base_version'] == 7 and platform_family?("rhel")
	remote_file "#{Chef::Config[:file_cache_path]}/tomcat7-#{node['tomcat']['package']['full_version']}.x86_64.rpm" do
		source "https://s3.amazonaws.com/#{node['tomcat']['package']['s3_bucket_path']}/tomcat7-#{node['tomcat']['package']['full_version']}.x86_64.rpm"
		action :create
	end
    #Install tomcat7
    package 'tomcat7' do
        action :install
        source "#{Chef::Config[:file_cache_path]}/tomcat7-#{node['tomcat']['package']['full_version']}.x86_64.rpm"
    end
else
